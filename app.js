(function () {
  ...
  const vault = window.DISNEY_VAULT || [];
  ...
  function ensureSchedule(nowMs) {
    ...
    try {
      ...
      schedule = [];
      showMessage("DISNEY", "The vintage Disney sources are being refreshed.", "Please try the channel again shortly.");
    }
  }
  ...
  function showStationCard(state) {
    ...
    if (state.segment.kind === "commercial") {
      showMessage("COMMERCIAL BREAK", state.segment.title, `${formatDuration(state.movieReturnsIn)} until the program returns`);
    } else if (state.segment.kind === "station") {
      showMessage("DISNEY", state.segment.title, `${formatDuration(state.segmentRemaining)} until the next program`);
    } else if (failedMovieVideoIds.has(state.segment.videoId)) {
      showMessage("SOURCE UNAVAILABLE", state.block.movie.title, `The schedule stays synchronized. Next program starts at ${formatStationTime(state.block.endsAtMs)}.`);
    } else {
  ...
  function tick() {
    ...
    els.mode.textContent = mode === "live" ? (state.segment.kind === "commercial" ? "LIVE · COMMERCIAL BREAK" : "DISNEY VINTAGE") : "TIME SHIFTED";
    ...
    els.position.textContent = mode === "live" ? "Synced with the Disney Vintage schedule" : `${formatDuration(state.blockElapsed)} from start`;
    els.remaining.textContent = `${formatDuration(state.blockRemaining)} until next scheduled program`;
  ...
  async function shareChannel() {
    const title = els.title.textContent && !els.title.textContent.includes("Loading") ? els.title.textContent : "DISNEY";
    const share = { title:`${title} · Disney Vintage`, text:`Watch ${title} on Disney Vintage.`, url:location.href };
    try {
      if (navigator.share) {
        ...
        els.shareStatus.textContent = "Disney Vintage link copied.";
      }
    } catch (error) {
    ...
    player = new YT.Player("player", {
      ...
      events:{
        ...
        onError:() => {
          ...
          // Hold the slot and move everyone to the next program at the same time.
          setTimeout(tick, 150);
        }
      }
    });
